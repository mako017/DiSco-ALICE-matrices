...
function clearOptions() {
	for (let i = 0; i < 20; i++) {
		...
		document
			.getElementById("opt-el" + (i + 1))
			.classList.remove("optionen-element-check");
	}
}

function fillMat(i) {
	if (Settings.constCode[Settings.constId.split("-el")[1] - 1][i] === "0") {
		Settings.constCode[Settings.constId.split("-el")[1] - 1] = replaceAt(
			Settings.constCode[Settings.constId.split("-el")[1] - 1],
			i,
			"1"
		);
	} else {
	...
	for (
		let j = 0;
		j < Settings.constCode[Settings.constId.split("-el")[1] - 1].length;
		j++
	) {
		if (
			Settings.constCode[Settings.constId.split("-el")[1] - 1].charAt(j) === "1"
		) {
			selShape(cvs, j);
		}
	}
...
function getResponse() {
	...
	if (Settings.item + 1 < items.length) {
		...
		document.getElementById("item-position").innerHTML =
			"Item " + (Settings.item + 1) + " von " + items.length;
		resetTime();
	} else {
...
for (let i = 1; i < 21; i++) {
	document.getElementById("opt-el" + i).addEventListener(
		"click",
		function (e) {
	...
	document.getElementById("cb-el" + i).addEventListener(
		"click",
		function () {
			toggleCvs("opt-el" + i);
			if (Settings.wm) fillMat(i - 1);
